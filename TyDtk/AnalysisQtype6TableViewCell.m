...
@interface AnalysisQtype6TableViewCell()<UIScrollViewDelegate,UIWebViewDelegate>
//??????????????????????????????????????????????????????
...
@end
@implementation AnalysisQtype6TableViewCell
...
//    //保存答案，刷新答题卡
//    [self.delegateAnalysisCellClick topicCellSelectClickTest:_indexTopic selectDone:nil isRefresh:YES];
//}
//图片的点击手势
...
- (void)webViewDidFinishLoad:(UIWebView *)webView{
    ...
    if (_isWebFirstLoading) {
        ...
        [self.delegateAnalysisCellClick isWebLoadingCellHeight:cellHeightL+_viewImage.frame.size.height + 60 withImageOy:cellHeightL];
    }
